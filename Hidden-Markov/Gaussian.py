...
def build_dataset(sound_path='digits-raw/'):
    ...
    for f in files:
        feature = mfcc_feature_extractor(sound_path=sound_path + f)
        # print(feature)
        if i % 10 == 0:
...
def mfcc_feature_extractor(sound_path):
    (sampling_freq, audio) = wav.read(sound_path)
    mfcc_features = mfcc(audio, sampling_freq,numcep=16)
...
def train_model(data):
    ...
    for label in data.keys():
        model = hmm.GMMHMM(n_components=5,covariance_type='diag', transmat_prior=1.0,n_iter=100)
        feature = np.ndarray(shape=(1, 16))
...
# report(y_test, y_pred, show_cm=True)
test_input= x_test[4]
predict_input = hmm.GMMHMM.predict(X=test_input)
print(predict_input)